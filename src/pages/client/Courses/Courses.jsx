        ...
        <div className={`transition-discrete duration-400 ease-in-out p-2 bg-gray-50 ${isCollapsed ? '' : ''}`}>
            ...
            <div className="transition-all duration-600 ease-in-out grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {!isLoading && currentPosts.map((course) => (
                    <div key={course.course_id} className="relative bg-white rounded-lg shadow-md overflow-hidden transition-all duration-300 hover:shadow-xl">
                        <div className="relative group">
                            ...
                            <div className="absolute inset-0  bg-opacity-40 flex flex-col justify-center items-center p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 backdrop-brightness-50">
                                ...
                                <span style={{ 'outlineColor': 'black' }} className="text-white text-xs mb-2 font-semibold ">{course.content_duration} hours total</span>
                                <div className="text-white text-xs mb-2">
                                    <span>{formatNumber(course.num_lectures)} lectures</span>
                                    <span className="mx-2">•</span>
                                    <span>{formatNumber(course.num_subscribers)} students</span>
                                </div>
                                <NavLink to={`view_courses/${course.course_id}`} className="mt-2 bg-black hover:bg-primary hover:text-white text-white py-1 px-3 text-xs rounded-md transition-colors">
                                    View Course
                                </NavLink>
                            </div>
                        ...
                        <div className="p-4">
                            <div className="flex items-center mb-2">
                                <div className="flex items-center text-yellow-400 mr-2">
                                    <FaStar />
                                    <span className="ml-1 text-xs text-gray-700 font-medium">{(course.Rating * 10).toFixed(1)}</span>
                                </div>
                                <span className="text-xs text-gray-500">({formatNumber(course.num_reviews)} reviews)</span>
                            </div>

                            <h3 className="text-sm font-semibold text-gray-800 mb-2 line-clamp-2">{course.course_title}</h3>
